...
/* ─────────────────────────────────────────────────────────
 * THINKING STATE — expandable agent trace adapted for KSP Datathon
 *
 * Variants:
...
const TONES = ['var(--color-service-blue)', 'var(--color-signal-amber)', 'var(--color-verified-green)'];
...
export default function ThinkingState({ variant = 'Steps' }: { variant?: string }) {
  ...
  return (
    <div key={variant} className="flex min-h-[176px] w-full max-w-95 flex-col">
      ...
      >
        <svg width="16" height="16" viewBox="0 0 24 24" fill={working ? 'var(--color-service-blue)' : 'var(--color-tertiary)'}>
        ...
        {working ? (
          <span
            ...
            style={{
              backgroundImage:
                "linear-gradient(90deg, var(--color-tertiary) 35%, var(--color-service-blue) 50%, var(--color-tertiary) 65%)",
          ...
          <span
            className="text-[13px] font-medium whitespace-nowrap text-tertiary"
        ...
        <svg
          width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--color-tertiary)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"
          ...
          <div className="relative mt-1 ml-[5px] pl-4">
            ...
            {v.query && (
              <div className="flex h-6 items-center gap-2 px-1.5" style={{ animation: expanded ? 'fade-up 300ms cubic-bezier(0.23,1,0.32,1) both' : undefined }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--color-tertiary)" strokeWidth="2" strokeLinecap="round" className="shrink-0">
                  <circle cx="11" cy="11" r="7" />
                  <path d="M21 21l-4.3-4.3" />
                </svg>
                <span className="text-[12.5px] text-tertiary">{v.query}</span>
              </div>
            )}
            {v.rows.slice(0, visible).map((row, i) => {
              const content = (
                ...
                {variant === "Steps" && (
                  i < visible - 1 || !working ? (
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--color-tertiary)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="shrink-0">
                      <path d="M20 6L9 17l-5-5" />
                    </svg>
                  ) : (
                    <span className="size-3 shrink-0 rounded-full border-[1.5px] border-tertiary border-t-service-blue" style={{ animation: 'spin 700ms linear infinite' }} />
                  )
                )}
                <span className={`min-w-0 truncate text-[12.5px] ${variant === "Reasoning" ? "whitespace-normal leading-relaxed text-tertiary" : "font-medium text-service-blue"} ${variant === "Search" ? "animated-underline" : ""}`}>
                  {row.primary}
                </span>
                {row.secondary && (
                  <span className={`shrink-0 text-[11.5px] text-tertiary ${row.mono ? "font-mono" : ""}`}>
                    {row.secondary}
                  </span>
                )}
                {row.add !== undefined && (
                  <span className="shrink-0 font-mono text-[11px] tabular-nums">
                    <span className="text-verified-green">+{row.add}</span>{" "}
                    <span className="text-alert-red">−{row.del}</span>
                  </span>
                )}
                </>
              );
            ...
            {variant === "Search" && stage >= 3 && (
              <span className="text-[12px] text-tertiary" style={{ animation: 'fade-in 300ms ease-out both' }}>
                +7 more
              </span>
            )}
            </div>
