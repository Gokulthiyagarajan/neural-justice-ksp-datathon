  ...
  return (
    <>
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-[10px] font-mono font-medium" style={{ color: RED }}>
                ...
                {districtMapData.map((d) => {
                  ...
                  return (
                    <button
                      key={d.name}
                      type="button"
                      className="rounded-md flex flex-col items-center justify-center text-center p-1 transition-all hover:scale-105 cursor-pointer border"
                      style={{
                    ...
                    >
                      <span className="text-[8px] font-medium text-text-secondary leading-tight truncate w-full">
        ...
        <SectionCard title="Division Breakdown" icon={Building2}>
          <div className="space-y-2.5">
            {(data?.division_breakdown ?? []).map((div: DivisionBreakdown) => (
              <div
                key={div.id}
                className="rounded-lg p-2.5 cursor-pointer transition-all hover:scale-[1.01] border"
        ...
        <SectionCard title="Top Districts — Solved Rate" icon={TrendingUp}>
          <div className="space-y-3">
            {(data?.top_districts_solved ?? []).map((d: DistrictSolvedRate) => (
              <div
                key={d.id}
                className="p-2.5 rounded-lg border cursor-pointer transition-all hover:scale-[1.005]"
                style={{ background: 'var(--bg-tertiary)', borderColor: 'var(--border-secondary)' }}
                onClick={() => {
                  drawer.open({
                    title: d.name,
